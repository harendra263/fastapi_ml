                ...
                if tvars is not None:
                    ...
                    if gvars is None:
                        ...
                        if not tvarset <= gvarset:
                            raise TypeError(
                                f'Some type variables ({", ".join(str(t) for t in tvars if t not in gvarset)}) are not listed in {"Generic" if any(b.__origin__ is Generic for b in bases) else "Protocol"}[{", ".join(str(g) for g in gvars)}]'
                            )
...
if sys.version_info >= (3, 9, 2):
    ...
    class _TypedDictMeta(type):
        def __init__(self, name, bases, ns, total=True):
            ...
            super(_TypedDictMeta, self).__init__(name, bases, ns)
